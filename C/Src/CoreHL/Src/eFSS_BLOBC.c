...
static e_eFSS_BLOBC_RES eFSS_BLOBC_HLtoBLOBRes(const e_eFSS_UTILSHLPRV_RES p_eHLRes);
static e_eFSS_BLOBC_RES eFSS_BLOBC_CrcBlobFull(t_eFSS_BLOBC_Ctx* const p_ptCtx, uint8_t* p_puBlob, uint32_t p_uBlobL,
                                               uint32_t* p_puBlobCrc);
static e_eFSS_BLOBC_RES eFSS_BLOBC_OriginBackupAligner(t_eFSS_BLOBC_Ctx* const p_ptCtx);
static e_eFSS_BLOBC_RES eFSS_BLOBC_ArePagesValid(t_eFSS_BLOBC_Ctx* const p_ptCtx, uint32_t p_uStartIdx, uint32_t p_uNPages,
                                                 bool_t* p_pbAreValid, t_eFSS_TYPE_PageMeta* p_ptMeta, uint8_t p_uSubT);
static e_eFSS_BLOBC_RES eFSS_BLOBC_WriteBlobNPrm(t_eFSS_BLOBC_Ctx* const p_ptCtx, uint8_t* p_puBuff, uint32_t p_uBuffL,
		...
		else
		{
            ...
            else
            {
                ...
                if( ( e_eFSS_BLOBC_RES_OK == l_eRes ) || ( e_eFSS_BLOBC_RES_OK_BKP_RCVRD == l_eRes ) )
                {
                    ...
                    {
                        l_eRes = eFSS_BLOBC_HLtoBLOBRes(l_eHLRes);
                    }
                    else
                    {
	...
	else
	{
		...
		else
		{
            ...
            else
            {
                ...
                l_eRes = eFSS_BLOBC_CrcBlobFull(p_ptCtx, &l_uZero, 0u, &l_uCrcZeroBlob);
                ...
                if( e_eFSS_BLOBC_RES_OK == l_eRes )
                {
                    ...
                    l_eRes = eFSS_BLOBC_WriteBlobNPrm(p_ptCtx, &l_uZero, 1u, 0u,
                    ...
                    if( e_eFSS_BLOBC_RES_OK == l_eRes )
                    {
                        ...
                        l_eRes = eFSS_BLOBC_WriteBlobNPrm(p_ptCtx, &l_uZero, 1u,
		...
		else
		{
            ...
            else
            {
                ...
                if( ( e_eFSS_BLOBC_RES_OK == l_eRes ) || ( e_eFSS_BLOBC_RES_OK_BKP_RCVRD == l_eRes ) )
                {
                    ...
                    l_eRes = eFSS_BLOBC_CrcBlobFull(p_ptCtx, p_puBuff, p_uBuffL, &l_uCrcBlob);
                    ...
                    else
                    {
                        ...
                        /* Write first the original pages */
                        l_eResSub = eFSS_BLOBC_WriteBlobNPrm(p_ptCtx, p_puBuff, p_uBuffL, 0u,
                                                           ( p_ptCtx->tStorSett.uTotPages / 2u ), &l_tPagePrm);

                        if( e_eFSS_BLOBC_RES_OK != l_eResSub )
                        {
                             l_eRes = l_eResSub;
                        }
                        else
                        {
                            /* Now write backup pages */
                            l_tPagePrm.uPageSubType = EFSS_PAGESUBTYPE_BLOBBKP;
                            l_eResSub = eFSS_BLOBC_WriteBlobNPrm( p_ptCtx, p_puBuff, p_uBuffL,
                                                                ( p_ptCtx->tStorSett.uTotPages / 2u ),
                                                                ( p_ptCtx->tStorSett.uTotPages / 2u ), &l_tPagePrm);
        ...
        else
        {
            ...
            if( e_eFSS_BLOBC_RES_OK == l_eRes )
            {
                if( 0u != l_uUnusedByte )
                {
                    ...
                    while( ( l_uUnusedByte > 0u ) && ( e_eFSS_BLOBC_RES_OK == l_eRes ) )
                    {
...
static e_eFSS_BLOBC_RES eFSS_BLOBC_OriginBackupAligner(t_eFSS_BLOBC_Ctx* const p_ptCtx)
{
	...
	else
	{
        ...
        l_eRes = eFSS_BLOBC_ArePagesValid(p_ptCtx, 0u, ( p_ptCtx->tStorSett.uTotPages/2u ), &l_bIsOriginValid, &l_tMetaOri, EFSS_PAGESUBTYPE_BLOBORI);

        if( e_eFSS_BLOBC_RES_OK == l_eRes )
        {
            l_eRes = eFSS_BLOBC_ArePagesValid(p_ptCtx, ( p_ptCtx->tStorSett.uTotPages/2u ), ( p_ptCtx->tStorSett.uTotPages/2u ),
            ...
            if( e_eFSS_BLOBC_RES_OK == l_eRes )
            {
                ...
                else if( ( true == l_bIsOriginValid ) && ( false == l_bIsBackupValid ) )
                {
                    ...
                    while( (l_uI <= ( (p_ptCtx->tStorSett.uTotPages/2u) - 1u) )  && ( e_eFSS_BLOBC_RES_OK == l_eRes ) )
                    {
        ...
        else
        {
                   ...
                   ( e_eFSS_BLOBC_RES_OK == l_eRes ) )
            {
                ...
                l_eHLR = eFSS_UTILSHLPRV_ReadPageNPrm(&p_ptCtx->tCtxCb, l_uI,  l_puBuF1, l_uBuF1L,
                                                      &l_tPagePrm, p_ptCtx->tStorSett.uRWERetry);
                l_eRes = eFSS_BLOBC_HLtoBLOBRes(l_eHLR);
                if( e_eFSS_BLOBC_RES_OK == l_eRes )
                {
